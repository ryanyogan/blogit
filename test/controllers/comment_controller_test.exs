...
defmodule Blogit.CommentControllerTest do
  ...
  test "updates chosen resource and redirects when data is valid and logged in as author",
  %{conn: conn, user: user, post: post, comment: comment} do
    conn = login_user(conn, user) |> put(post_comment_path(conn, :update, post, comment),
  ...
  %{conn: conn, user: user, post: post, comment: comment} do
    conn = login_user(conn, user) |> delete(post_comment_path(conn, :delete,
          post, comment))
    assert redirected_to(conn) == user_post_path(conn, :show, post.user, post)
  ...
  test "updates chosen resource and redirects when data is valid, logged in as author",
  %{conn: conn, user: user, post: post, comment: comment} do
    conn = login_user(conn, user) |> put(post_comment_path(conn, :update, post, comment),
      comment: %{"approved" => true})
    assert redirected_to(conn) == user_post_path(conn, :show, user, post)
  ...
  %{conn: conn, post: post, comment: comment} do
    conn = put(conn, post_comment_path(conn, :update, post, comment),
      comment: %{"approved" => true})
    assert redirected_to(conn) == page_path(conn, :index)
